...
// Print helper: clone visible tab's table into a dedicated print area and trigger print
window.printVisibleTable = function () {
    try {
        let printArea = document.getElementById('tageepPrintArea');
        if (!printArea) {
            printArea = document.createElement('div');
            printArea.id = 'tageepPrintArea';
        ...
        if (ph) printArea.appendChild(ph.cloneNode(true));

        // find active tab and its table; fallback to any visible table
        let activeTab = document.querySelector('.tab-content.active');
        let table = activeTab ? activeTab.querySelector('table') : null;
        if (!table) {
            // try to find first table inside a visible .table-wrapper
            const wrappers = Array.from(document.querySelectorAll('.table-wrapper'));
            for (const w of wrappers) {
                const style = window.getComputedStyle(w);
                if (style.display !== 'none' && style.visibility !== 'hidden') {
                    const t = w.querySelector('table');
                    if (t) { table = t; activeTab = w; break; }
                }
            }
        }
        if (!table) {
            console.warn('printVisibleTable: no table found to print');
            alert('لا توجد جداول للطباعة في الواجهة الحالية');
            return;
        }

        // clone the table and ensure it is fully expanded
        const tableClone = table.cloneNode(true);
        tableClone.style.width = '100%';
        tableClone.style.maxHeight = 'none';
        tableClone.style.overflow = 'visible';
        // remove any inline styles that might limit rows
        tableClone.querySelectorAll('[style]').forEach(el => el.removeAttribute('style'));

        printArea.appendChild(tableClone);

        // add printing class and call print
        document.body.classList.add('printing');
        console.log('printVisibleTable: printing', { tableClone, activeTabId: activeTab?.id || null });
        ...
        setTimeout(() => {
            try {
                ...
                // cleanup
                setTimeout(() => { printArea.innerHTML = ''; }, 200);
            }
        }, 120);
    } catch (err) {
